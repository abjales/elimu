...
const getDefaultAudioConfig = () => ({
  ...
  ttsProvidersConfig: {
    ...
    // Browser-native is ON by default for the Elimu deployment (no TTS API key
    // configured yet), so the classroom has audible voices out of the box.
  ...
  persist(
    (set, get) => {
      ...
      return {
        ...
        // TTS ON by default for the Elimu deployment (browser-native voice), so
        // the classroom is audible without a server TTS provider configured.
        ttsEnabled: true,
        asrEnabled: true,
          ...
          try {
            ...
            set((state) => {
              ...
              if (!state.autoConfigApplied) {
                ...
                // Auto-enable TTS whenever a server provider exists and TTS is off.
                // Runs on every server-providers fetch (not just first run) so that
                // adding a TTS key to .env.local takes effect immediately.
              ...
              // Always auto-enable TTS when a server provider exists but TTS is off.
              // This runs on every server-providers fetch (not just first run) so
              // that adding a TTS key to .env.local takes effect immediately
              // even if the user visited before the key was configured.
              const currentServerTtsIds = Object.entries(data.tts)
                .filter(([, info]) => !info.disabled)
                .map(([id]) => id);
              if (currentServerTtsIds.length > 0 && !state.ttsEnabled) {
                autoTtsEnabled = true;
                // Also select the first server TTS provider if current isn't server-configured
                if (!newTTSConfig[state.ttsProviderId]?.isServerConfigured) {
                  autoTtsProvider = currentServerTtsIds[0] as TTSProviderId;
                  autoTtsVoice =
                    DEFAULT_TTS_VOICES[autoTtsProvider as BuiltInTTSProviderId] || 'default';
                }
              }
    ...
    },
    {
      ...
      migrate: (persistedState: unknown, version: number) => {
        ...
        // v4 → v5: enable browser-native TTS by default so the classroom is
        // audible without a server TTS API key (Elimu deployment). Force-enable
        // any persisted state that predates this change.
        if (version < 5) {
          (state as Record<string, unknown>).ttsEnabled = true;
          const browserNative = state.ttsProvidersConfig?.['browser-native-tts'];
          if (browserNative) browserNative.enabled = true;
        }
