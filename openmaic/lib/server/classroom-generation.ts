...
/**
 * Wrap an `AICallFn` so that, when free-model rotation is active, both
 * quota/rate-limit errors AND empty successful responses are normalized into a
 * `QuotaExhaustedError`. Free models frequently return an HTTP 200 with an
 * empty body once their quota is exhausted (instead of a 429), which otherwise
 * surfaced as a plain parse error and hard-failed the whole job. Throwing
 * `QuotaExhaustedError` lets the job runner rotate to the next free model and
 * retry, matching the 429 path. No-op (passes through) when rotation is off.
 */
export function makeQuotaAwareAICall(base: AICallFn): AICallFn {
  if (!isFreeRotationActive()) return base;
  return async (systemPrompt, userPrompt, images) => {
    try {
      const text = await base(systemPrompt, userPrompt, images);
      if (text === null || text === undefined || String(text).trim() === '') {
        throw new QuotaExhaustedError(
          'LLM returned empty response (quota likely exhausted)',
        );
      }
      return text;
    } catch (err) {
...
export async function generateClassroom(
  ...
  // Quota-aware wrapper: when free-model rotation is active, normalize any
  // quota/rate-limit error OR empty successful response into a
  // QuotaExhaustedError so the job runner rotates + retries instead of failing.
  const aiCallQuotaAware = makeQuotaAwareAICall(aiCall);
  ...
  // Quota-aware variant for scene-content generation (rotation active).
  const sceneAiCallQuotaAware = makeQuotaAwareAICall(sceneAiCall);
  ...
  if (agentMode === 'generate') {
    ...
    try {
      agents = await generateAgentProfiles(requirement, languageDirective, aiCallQuotaAware);
  ...
  for (const [index, outline] of outlines.entries()) {
    ...
    const content = await withGenerationRetry(
      () =>
        generateSceneContent(safeOutline, sceneAiCallQuotaAware, {
          agents,
          languageDirective,
          allowProceduralSkill: vocationalActive,
        }),
    ...
    const actions = await withGenerationRetry(
      () =>
        generateSceneActions(safeOutline, content, sceneAiCallQuotaAware, {
          agents,
          languageDirective,
        }),
      {
